...
[预期结果] 1. 桌面端正常启动至主界面，初始化无中断、无错误弹窗。2. 应用启动后在后台自动向 https://github.com/nottyjay/archer-harness-desktop/releases.atom 发起一次实时检查（无缓存），判定最新 tag v0.6.6 高于本机 0.6.5，确认存在新版本
