...
- AppData layout（仅可变数据/辅助依赖）：`dependencies/pnpm/`、Windows 的 `dependencies/git/`、`.store.dat` / `.store.dev.dat`（后者为 debug）；服务日志 `logs/dsh-web.log`（debug 为 `logs/dsh-web.dev.log`）；`$DSH_HOME` 在用户主目录（release `~/.dsh`，debug `~/.dsh.dev`）。Node/DSH 固定从安装包 `resources/bundled/<target>/` 读取。
