...
- **数据隔离（核心只读、数据不共用）**：Node 与 DSH 从安装包 `resources/bundled/<distribution-os>-<arch>/` 直接运行，不复制到 AppData，也不在启动时下载；AppData 只存放 pnpm、Windows MinGit 等可变辅助依赖。官方 DSH 仍默认 `~/.dsh`；桌面在进程启动时注入 `$DSH_HOME`（release 未设置时为 `~/.archer`，debug 固定 `~/.archer.dev`），不改 vendor。打包时把 `patches/dsh/*.patch` 打到 `.tmp` 暂存副本再编译，vendor 保持上游原版；Harness 的 `packageManager`（pnpm@11.7.0）必须由内置 Node 的 corepack 兑现，禁止回退桌面仓库的 pnpm 10。debug 的 store 用独立文件 `.store.dev.dat`（`config::setting::store_dat_file_name`），避免开发版与生产版会话/档案/端口状态互相污染，也防止 dev 版热重启把 release 的服务进程杀掉（`service/workflow::terminate_stale_harness_processes` 在 debug 下为 no-op，改由 `.archer.dev/.harness.pid` 精确回收）。debug 构建不迁移旧数据（`service/migrate`）、不注册/注销 PATH、不写烘焙 DSH_HOME 的 `dsh` shim（`service/cli`）。
...
- **Backend**: Rust / Tauri 2 (`src-tauri/src/`)
  ...
  - `service/download/`: pnpm/Windows MinGit 辅助依赖安装，以及用户主动操作的历史核心版本下载；启动流程不下载 Node/DSH
...
├── hooks/                # 可复用 hook（use-iframe-post / use-iframe-message / use-invoke-iframe / use-listen …）
...
- `dsh` CLI 是随包资源中的 Node 脚本（`resources/bundled/<target>/dsh/node_modules/@deepseek-ai/dsh/lib/bin.js`）；应用服务固定使用同目录内的 Node。pnpm 仍是 AppData `dependencies/pnpm/bin/pnpm.cjs` 中的 JS 辅助依赖。
- AppData layout（仅可变数据/辅助依赖）：`dependencies/pnpm/`、Windows 的 `dependencies/git/`、`.store.dat` / `.store.dev.dat`（后者为 debug）；服务日志 `logs/dsh-web.log`（debug 为 `logs/dsh-web.dev.log`）；`$DSH_HOME` 在用户主目录（release `~/.archer`，debug `~/.archer.dev`）。Node/DSH 固定从安装包 `resources/bundled/<target>/` 读取。
