...
Release installers include their target Node.js and compiled Harness runtime in
`resources/bundled/<os>-<arch>`. The application executes those read-only
resources directly; it does not copy them to AppData or download them after
installation. The AppData directory remains available for legacy installs and
writable supporting tools:

- `runtime/` — legacy Node.js runtime location
- `dependencies/dsh/` — legacy Harness installation location
