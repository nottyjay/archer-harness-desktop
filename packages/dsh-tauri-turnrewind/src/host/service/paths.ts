/**
 * host/service/paths.ts — 恢复路径的安全解析与删除。
 *
 * 撤销是**唯一会写用户工作区**的动作，因此路径判定必须在词法之上再加一层文件系统
 * 事实校验：`sub/file.txt` 词法上在工作区内，但 `sub` 若是指向外部的符号链接或
 * Windows junction，`git checkout` / `unlink` 就会写到工作区外面去。
 *
 * 两条规则：
 *   1. 只检查**父级**：目标自身若是符号链接，`unlink` / 覆盖只作用于链接本身，
 *      不会穿透（这是安全且符合用户预期的）；
 *   2. 用 `lstat` 而不是 `existsSync` 预检：`existsSync` 对**悬空链接**返回 false，
 *      而悬空链接恰恰是最不能当作"不存在"处理的路径组件。
 */

import type { PathSafety } from '../types'
...
/**
 * 词法包含检查：把相对路径解析到工作区内，越界（绝对路径、`..` 逃逸、NUL）返回 null。
 * @param worktree - 工作区根。
 * @param path - git 输出的相对路径。
 * @returns 绝对路径，或 null（越界）。
 */
 ...
 * @param worktree - 工作区根。
 * @param absolutePath - 由 {@link resolveInsideWorkspace} 得到的绝对路径。
 * @returns 通过或拒绝原因。
 */
 ...
 * @param absolutePath - 已通过 {@link assertSafeParents} 的绝对路径。
 * @returns 删除结果与拒绝原因。
 */
