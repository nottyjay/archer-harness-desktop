...
const DICT_ZH = {
  ...
  unsafeWorkspaceReason: '当前工作区位于系统目录（如家目录或盘根），出于安全考虑不做快照，无法撤销。',
  tooManyFilesReason: '工作区内参与快照的文件数超过单次快照上限，这一轮不提供撤销。把生成产物等无关目录加入 .gitignore 可避免该提示。',
  snapshotTooLargeReason: '本轮涉及的文件总体积超过单次快照上限，这一轮不提供撤销。',
  tooManyOversizedReason: '工作区内超大文件过多、快照无法逐一排除，这一轮不提供撤销。',
  alreadyUndoneReason: '这一轮已经被撤销过，无需重复操作。',
  conflictReason: '撤销前相关文件又被修改，撤销已被拒绝，未改动任何文件。',
  nonEmptyDirReason: '撤销目标是一个非空目录，撤销不会递归删除目录。',
...
const DICT_EN: Record<LocaleKey, string> = {
  ...
  unsafeWorkspaceReason: 'The workspace is inside a system location (such as the home directory or a drive root); snapshots are refused for safety, so this turn cannot be undone.',
  tooManyFilesReason: 'The workspace contains too many files for a single snapshot, so this turn cannot be undone. Ignoring generated output in .gitignore avoids this notice.',
  snapshotTooLargeReason: 'The files involved in this round exceed the per-snapshot size limit, so this turn cannot be undone.',
  tooManyOversizedReason: 'Too many oversized files to exclude individually, so this turn cannot be undone.',
  alreadyUndoneReason: 'This turn has already been undone.',
  conflictReason: 'The files changed again before the undo; the undo was refused and nothing was modified.',
  nonEmptyDirReason: 'The undo target is a non-empty directory; undo never deletes directories recursively.',
